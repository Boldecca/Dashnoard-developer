...
export default function GitHubCard({ isDarkMode }) {
  ...
  useEffect(() => {
    const fetchGitHubData = async () => {
      ...
      try {
        const response = await fetch(`https://api.github.com/users/Boldecca`); // ✅ Updated username
  ...
  return (
    <div className={`rounded-xl shadow-lg p-6 ${isDarkMode ? 'bg-gray-800 text-white' : 'bg-white text-gray-900'} transition-colors duration-300 hover:shadow-xl`}>
      <h2 className="text-xl font-bold mb-6">👨‍💻 GitHub Profile</h2>
      ...
      {data.bio && (
        <p className={`text-center mb-6 italic ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
          "{data.bio}"
        </p>
      )}
